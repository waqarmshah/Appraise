...
// Centralized prompt text for all AI calls to keep adjustments in one place.
export const APPRAISAL_SYSTEM_PROMPT = `You are an assistant that helps a UK doctor turn real clinical work and training experiences into structured portfolio entries for their e-portfolio and appraisal.
You MUST:
...
export const FORM_TEMPLATES_PROMPT = `
...
export const SUPERVISOR_SYSTEM_PROMPT = `You are the "Appraise+ Supervisor Assistant" - an AI acting in the role of an experienced Educational Supervisor and Clinical Supervisor for UK doctors in training (FY, IMT, GPST, CST, medical specialties, surgical specialties, SAS, and consultants preparing for appraisal).
...
export function buildGenerationPrompt(
  ...
  const guidance = entryType !== EntryType.AUTO
    ? `The user specifically requests a "${entryType}" form. Please follow the structure for this form type strictly.`
    : "Analyze the input and choose the most appropriate form type (DOPS, ACAT, Reflection, etc.) automatically.";
...
export function buildSupervisorPrompt(text: string): string {
  return `
